...
export default function OfferUpdateChart() {
    ...
    const chartData =
        data?.map((item) => ({
            date: new Date(item.dateOfUpdate).toLocaleDateString("pl-PL", {
                day: "2-digit",
                month: "short",
            }),
    ...
    return (
        <div className="bg-gradient-admin-info rounded-xl shadow-lg p-6">
            <div className="h-80 min-h-[300px] w-full min-w-0 overflow-visible pr-8">
                <ResponsiveContainer width="100%" height="100%" minHeight={300} minWidth={0}>
                    <LineChart data={chartData} margin={{ top: 10, right: 56, left: 0, bottom: 0 }}>
                        <defs>
                        ...
                        />
                        <Legend
                            wrapperStyle={{ paddingTop: "20px", color: "#e0f7ff" }}
                            iconType="line"
